 ...
 * example, suppose a before-action registers using the pattern "/*seg" and
 * there's also a route "/hello/:seg". For an inbound request "/hello/world",
 * the former's "seg" parameter will map to the value "/hello/world" and the
 ...
 * action, will always observe the expected path parameter value even if at that
 * point in time the invocation chain has already proceeded to the request
 * handler.<p>
 * 
 ...
 * and body. Therefore, changes to these structures propagate across execution
